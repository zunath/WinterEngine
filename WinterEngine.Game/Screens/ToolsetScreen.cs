...
namespace WinterEngine.Game.Screens
{
    public partial class ToolsetScreen
    {
        ...
        {
            // Tileset Editor
            OnTilesetLoaded += TilesetEditorEntityInstance.HandleLoadTilesetSpritesheetEvent;
            OnTilesetSaved += TilesetEditorEntityInstance.HandleSaveTilesetSpritesheetEvent;
        }

        private void HandleModeChangeEvent(GameObjectTypeEnum gameObjectType)
        {
            HideAllEditors();
        ...
        private void HandleAreaLoadEvent(int resourceID)
        {
            Area selectedArea;
            using (AreaRepository repo = new AreaRepository())
            {
                selectedArea = repo.GetByID(resourceID);
            }
        ...
        {
            HandleAreaLoadEvent(e.ResourceID);
        }
        ...
        private void ChangeObjectMode(object sender, JavascriptMethodEventArgs e)
        {
            if (ViewModel.IsModuleOpened)
            {
                ...
                HandleModeChangeEvent((GameObjectTypeEnum)Enum.Parse(typeof(GameObjectTypeEnum), mode));
